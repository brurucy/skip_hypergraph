...
class LookupWorkers():
    ...
    def run(self, job, iterable, nr):
        self.pool.map_async(partial(job, nr=nr), iterable, 
                            callback=self.callback, chunksize=len(iterable)/cpu_count())
...
#     def lookup(self, batch_indices, nr):
#         # heights is global
#         while 1:
#             for he in heights[batch_indices]:
...
#             batch_indices, nr = self.in_queue.get()


#     def run(self, batch_indices, nr):
#         result = self.lookup(batch_indices, nr)
...
#             worker = self.workers[i] or LookupProcess(self.queue)
#             worker.start()
#             self.workers.append(process)


#         results = self.pool.map_async(
#             partial(job, nr=nr), iterable, callback=self.callback)
...
class SplitList:  # Rucy, rename it!
    ...
    # def lookup_subprocess(indices, nr):
    #     for he in heights[indices]:
    #         if nr <= he.sublists[-1].max:
    #             i = bisect.bisect_left(he.sublists, nr)
    ...
    #     indices = range(len(self.blists))
    #     global heights
    #     heights = np.array(self.blists)
    #     result = workers.run(self.lookup_subprocess, indices, nr)

    #     return result

    def lookup_subprocess(he, nr):
        if nr <= he.sublists[-1].max:
            i = bisect.bisect_left(he.sublists, nr)
